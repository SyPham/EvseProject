...
@Component({
  selector: 'app-engineer-error-report',
  templateUrl: './engineer-error-report.component.html',
  styleUrls: ['./engineer-error-report.component.scss']
})
export class EngineerErrorReportComponent extends BaseComponent implements OnInit {
  data: DataManager;
  modalReference: NgbModalRef;
  active = "Detail"

  @ViewChild('grid') public grid: GridComponent;
  model: EngineerErrorReport = {} as EngineerErrorReport;
  ...
  loadLang() {
    this.translate.get('EngineerErrorReport').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}EngineerErrorReport/LoadData?lang=${lang}`,
  ...
  cancel() {
    this.model = {} as EngineerErrorReport;
    this.audit = {}
  }
  rowSelected(args) {
  ...
  fnEdit(data) {
    let link = `/evse/engineer-error-report/action/${data.id}`;
    this.router.navigateByUrl(link)
  }
  fnAdd() {
    let link = `/evse/engineer-error-report/action/${0}`;
    this.router.navigateByUrl(link)
  }
  fnSearch() {
