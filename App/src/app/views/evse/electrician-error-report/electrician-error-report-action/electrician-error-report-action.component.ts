...
@Component({
  selector: 'app-electrician-error-report-action',
  templateUrl: './electrician-error-report-action.component.html',
  styleUrls: ['./electrician-error-report-action.component.css']
})
export class ElectricianErrorReportActionComponent implements OnInit {
  ...
  loading
  model: ElectricianErrorReport = <ElectricianErrorReport>{};
  file: any;
...
id: any;
  ...
  back() {
      this.router.navigateByUrl("/evse/electrician-error-report")
  }
  ...
  loadCodeTypeViewError() {
    ...
    this.dataSourceCodeTypeViewError = new DataManager({
      url: `${environment.apiUrl}CodeType/GetDataDropdownlist?lang=${localStorage.getItem('lang')}&codeType=Electrician_ViewError`,
