...
import { ElectricianErrorReport } from 'src/app/_core/_model/evse/model';
import { ElectricianErrorReportService } from 'src/app/_core/_service/evse/electrician_error_report.service';
...
@Component({
  selector: 'app-electrician-error-report',
  templateUrl: './electrician-error-report.component.html',
  styleUrls: ['./electrician-error-report.component.scss']
})
export class ElectricianErrorReportComponent extends BaseComponent implements OnInit {
  data: DataManager;
  modalReference: NgbModalRef;
  active = "Detail"

  @ViewChild('grid') public grid: GridComponent;
  model: ElectricianErrorReport = {} as ElectricianErrorReport;
  ...
  electricianErrorReportData: any = [];
  ...
  constructor(
    private service: ElectricianErrorReportService,
  ...
  loadLang() {
    this.translate.get('ElectricianErrorReport').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}ElectricianErrorReport/LoadData?lang=${lang}`,
  ...
  cancel() {
    this.model = {} as ElectricianErrorReport;
    this.audit = {}
  }
  rowSelected(args) {
  ...
  fnEdit(data) {
    let link = `/evse/electrician-error-report/action/${data.id}`;
    this.router.navigateByUrl(link)
  }
  fnAdd() {
    let link = `/evse/electrician-error-report/action/${0}`;
    this.router.navigateByUrl(link)
  }
  fnSearch() {
