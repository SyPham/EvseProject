...
export class EngineerComponent extends BaseComponent implements OnInit {
  ...
  loadLang() {
    this.translate.get('Engineer').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Engineer/LoadData?lang=${lang}`,
  ...
  openModal(template, data = {} as Engineer) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'Engineer_Edit_Model';
    } else {
      this.model.id = 0;
