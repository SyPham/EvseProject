...
import { RecordAbattoir } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
import { NgxSpinnerService } from "ngx-spinner";
import { switchMap } from "rxjs/operators";
import { RecordMarketModalComponent } from "..";
import { RecordAccidentFeeModalComponent } from "../record-sale/record-accident-fee-modal/record-accident-fee-modal.component";
import { RecordAbattoirService } from "src/app/_core/_service/apply-orders/record-abattoir.service";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
declare let window: any;
@Component({
  selector: "app-record-abattoir",
  templateUrl: "./record-abattoir.component.html",
  styleUrls: ["./record-abattoir.component.css"],
})
export class RecordAbattoirComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  @ViewChild(RecordMarketModalComponent)
  marketModal: RecordMarketModalComponent;
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordAbattoir = new EventEmitter();
  data: DataManager;
  ...
  @ViewChild(RecordAccidentFeeModalComponent)
  public recordAccidentFeeModalComponent: RecordAccidentFeeModalComponent;

  model: RecordAbattoir;
  setFocus: any;
  ...
  @ViewChild("makeOrderGuidSearchTemplate", { static: true })
  public makeOrderGuidSearchTemplate: any;
  ...
  actionBeginPig(e: any) {
    ...
    if (e.action === "edit" && e.requestType === "save") {
      const index = this.selectedPigData.findIndex(
        (obj) => obj.pigGuid === e.data.pigGuid
      );
      if (index !== -1) {
  ...
  constructor(
    public service: RecordAbattoirService,
  ...
  @ViewChild("customerTemplate", { static: true }) public customerTemplate: any;
  ...
  ngOnInit() {
    ...
    this.toolbarOptions = [
      ...
      { template: this.customerTemplate, align: "Left" },
    ];
  ...
  onChangeCustomerGuidSearch(e) {
    if (e.isInteracted) {
      this.customerGuidSearch = e.value;
      this.loadData();
    }
  }
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Record_Abattoir")
      .toPromise();
  }
  async loadPen() {
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Record_Abattoir");
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordAbattoir(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Record_Abattoir");
  ...
  loadData() {
    ...
    if (this.customerGuidSearch) {
      this.query.where("customerGuid", "equal", this.customerGuidSearch);
    }
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordAbattoir/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
      this.query
  ...
  getCheckedPigData() {
    this.servicePig
      .getSelectedPigsByRecord(this.model?.guid, "Record_Abattoir")
      .subscribe((data: any) => {
  ...
  setStatus(model: RecordAbattoir) {
  ...
  goToRecordmarket() {
    this.marketModal.makeOrderGuid = this.model.makeOrderGuid;
    this.marketModal.saleGuid = this.model.guid;
    this.marketModal.pigs = this.selectedPigData.map(x=> {
      return {
        pigNo: x.name,
        sex: x.pigSexName
      }
    });
    this.marketModal.open();
  }
  accidentCalculate() {
    this.recordAccidentFeeModalComponent.open();
  }
  closeChange(e) {
    this.service.accidentCalculate(this.model.guid).subscribe((item) => {
      if (item) {
        this.model.accidentFee = item;
      }
    });
    this.service
    .accidentCalculate(this.model.guid)
    .pipe(
      switchMap((res) =>  res ?  this.service.getById(this.model.id): of(null) )
    )
    .subscribe(item => {
      if (item) {
        this.model.accidentFee = item.accidentFee;
      }
    });
