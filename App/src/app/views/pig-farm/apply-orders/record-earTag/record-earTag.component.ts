  ...
  // life cycle ejs-grid
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        {
          new DataManager({
            url: `${this.baseUrl}CodeType/GetDataDropdownlist?lang=${localStorage.getItem('lang')}&codeType=Record_EarTag_Type`,
            adaptor: new UrlAdaptor,
            crossDomain: true,
          }).executeQuery(new Query().where("guid", "equal", this.model.type)).then( (res: any) => {
            const data = res.result;
            for (const item of data) {
              if (item.guid === this.model.type) {
                this.service.changLable(item.name)
              }
            }
          });
