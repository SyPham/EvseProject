  ...
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    private record2TreatmentService: Record2TreatmentService,
  ) {
 ...
 // call or add here your code
  ...
  loadWarningText() {
    let query = new Query();
    const accessToken = localStorage.getItem("token");
    let predicate: Predicate = new Predicate("type", "equal", "Warning");
    predicate = predicate.and("no", "equal", "disinfection");
    ...
    new DataManager({
      ...
      .executeQuery(query)
      .then((x: any) => {
        const data = x.result || ([] as any[]);
        this.model.warningText = data.length > 0 ? data[0].value : "";
      });
  }
  onChangeDisinfection(e) {
    this.model.warningText = e.itemData?.comment;
