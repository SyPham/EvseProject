...
import { RecordAccidentFee } from 'src/app/_core/_model/model';
...
@Component({
  selector: 'app-record-accident-fee',
  templateUrl: './record-accident-fee.component.html',
  styleUrls: ['./record-accident-fee.component.scss']
})
export class RecordAccidentFeeComponent extends BaseComponent implements OnInit {
  data: DataManager;
  modalReference: NgbModalRef;

  @ViewChild('grid') public grid: GridComponent;
  model: RecordAccidentFee;
  ...
  loadLang() {
    this.translate.get('RecordAccidentFee').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}RecordAccidentFee/LoadData?farmGuid=${farmGuid}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as RecordAccidentFee).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as RecordAccidentFee) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'RecordAccidentFee_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'RecordAccidentFee_ADD_MODAL';
    }
