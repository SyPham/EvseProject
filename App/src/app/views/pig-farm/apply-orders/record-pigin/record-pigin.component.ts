...
import {
  RecordPigInService ,Record2PigService, Record2PigInService,
} from "src/app/_core/_service/apply-orders";
import { Record2PigIn, RecordPigIn } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
import { Record2PiginComponent } from './record-2pigin/record-2pigin.component';
...
export class RecordPiginComponent
  ...
  record2PigInsData: Record2PigIn[];
  realTotalWeight: any;
  realQty: any;
  @ViewChild(Record2PiginComponent, {static: false}) record2PiginComponent: Record2PiginComponent
...
private router: Router,
    ...
    public service2: Record2PigInService,
    public translate: TranslateService,
    ) {
      super(translate,environment.apiUrl);
    }
  ngAfterViewInit(): void {
    this.subscription.add(this.service2.currentRecord2PigIn.subscribe(x=> {
      if (JSON.stringify(x) !== JSON.stringify(this.record2PigInsData )) {
        this.record2PigInsData = x as any;
      }
    }));

    this.subscription.add(this.service2.realTotalWeightCurrent.subscribe(x=> {
      if (x) {
        this.realTotalWeight = x as any;
      }
    }));

    this.subscription.add(this.service2.realQtyCurrent.subscribe(x=> {
      if (x) {
  ...
  // life cycle ejs-grid
    ...
    create() {
      ...
      this.alertify.confirm4(
        ...
        () => {
          ...
          this.model.record2PigIns = this.record2PigInsData;
    ...
    veterinary() { 
      ...
      this.model.realQty = this.realQty
        });
    }
    execute() {
    ...
    async openModal(template, data = {} as any) {
      ...
      this.record2PigInsData = [] as Record2PigIn[];
