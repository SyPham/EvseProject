...
@Component({
  selector: 'app-apply-order-config',
  templateUrl: './apply-order-config.component.html',
  styleUrls: ['./apply-order-config.component.scss']
})
export class ApplyOrderConfigComponent extends BaseComponent implements OnInit, OnChanges {
  systemMenuGuid:any;
  @Input() topHeight;
...
public commonFn: (args: { [key: string]: string }) => boolean = (args: { [key: string]: string }) => {
  ...
  rowSelected(args) {
    this.systemMenuGuid =args.data.guid;
  }
  actionBegin(args) {
...
toolbarClick(args) {
  ...
  loadData() {
    ...
    this.query.where("menuLink", "contains", '/apply-orders');
    this.query.where("status", "equal", '1');
    this.data = new DataManager({
      url: `${this.baseUrl}SysMenu/LoadData`,
