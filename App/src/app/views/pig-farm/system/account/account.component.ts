  ...
  actionBegin(args) {
    ...
    if (args.requestType === 'save' && args.action === 'add') {
      ...
      if (args.data.username === undefined) {
        this.alertify.error(this.translate.instant('Please key in a xAccount!'));
        args.cancel = true;
        return;
      }
      if (args.data.password === undefined) {
        this.alertify.error(this.translate.instant('Please key in a password!'));
        args.cancel = true;
        return;
      }
...
toolbarClick(args) {
  ...
  storePermiison() {
    this.alertify.confirm4(
      this.alert.yes_message,
      this.alert.no_message,
      this.translate.instant('Store Permission'),
      this.translate.instant('Are you sure you want to store this?'),
      () => {
        const request = {
