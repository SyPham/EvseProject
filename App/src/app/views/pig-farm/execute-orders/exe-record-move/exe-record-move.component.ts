...
public actionCompleteAfterPen(e: any): void {
  e.result = e.result?.filter(x=> x?.guid != "");
}
public onFilteringAfterPen: any = (e: any) => {
  let query: Query = new Query().skip(this.skip)
  .take(this.take)
  .where("roomGuid", "equal", this.model.afterRoomGuid)
  .where("farmGuid", "equal", localStorage.getItem("farmGuid"))
  .where("status", "equal", 1)
  //frame the query based on search string with filter type.
  query = (e.text !== '') ? query.search(e.text, ['penName', 'penNo']) : query;
  //pass the filter data source, filter query to updateData method.
  e.updateData(this.dataPenForAfterRoom, query);
};
...
private router: Router,
  ...
  loadAfterPen() {
    this.servicePen.getPensByFarmGuidAndRoomGuid(localStorage.getItem("farmGuid"), this.model.afterRoomGuid).subscribe(x=> {
        this.dataPenForAfterRoom = x;
        console.log(x)
      });
  }
  loadPig() {
  ...
  onChangeAfterRoom(e) {
    this.model.afterRoomGuid = e.value;
    this.loadAfterPen();
  }
  onChangeRoom(e) {
  ...
  loadData() {
    ...
    const deleteStatus = 99;
    this.query.where("status", "notequal", deleteStatus);
