...
import { AbattoirLevel } from 'src/app/_core/_model/model';
import { AbattoirLevelService } from 'src/app/_core/_service/abattoir-level.service';

declare let window:any;

@Component({
  selector: 'app-abattoir-level',
  templateUrl: './abattoir-level.component.html',
  styleUrls: ['./abattoir-level.component.css']
})
export class AbattoirLevelComponent extends BaseComponent implements OnInit {
  data: DataManager;
  ...
  constructor(
    private service: AbattoirLevelService,
  ...
  loadLang() {
    this.translate.get('AbattoirLevel').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}AbattoirLevel/LoadData?farmGuid=${farmGuid}`,
  ...
  openModal(template, data = {} as AbattoirLevel) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'AbattoirLevel_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'AbattoirLevel_ADD_MODAL';
    }
