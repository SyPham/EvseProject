...
import { TransferFeeService } from 'src/app/_core/_service/transfer-fee.service';
import { environment } from 'src/environments/environment';
import { DatePipe } from '@angular/common';
import { TransferFee } from 'src/app/_core/_model/model';
...
@Component({
  ...
  styleUrls: ['./transfer-fee.component.scss']
})
export class TransferFeeComponent extends BaseComponent implements OnInit {
  data: DataManager;
  ...
  constructor(
    private service: TransferFeeService,
  ...
  loadLang() {
    this.translate.get('TransferFee').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}TransferFee/LoadData?farmGuid=${farmGuid}`,
  ...
  openModal(template, data = {} as TransferFee) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'TransferFee_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'TransferFee_ADD_MODAL';
    }
