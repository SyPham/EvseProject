...
import { Breeding2SowConst, SowFarrow } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SowService, Breeding2SucklingService, BreedingService, SowFarrowService } from "../../services";
declare let window: any;
@Component({
  selector: "app-sow-farrow",
  templateUrl: "./sow-farrow.component.html",
  styleUrls: ["./sow-farrow.component.scss"],
})
export class SowFarrowComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSowFarrow = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SowFarrowService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SowFarrow)
      .toPromise();
  }
  ...
  recordClick(args: any) {
    ...
    this.service.changeSowFarrow(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SowFarrow);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SowFarrow/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  addBorn() {
    this.alertify.confirm4(
      ...
      this.alert.updateMessage,
      () => {
        this.service.born({recordGuid: this.model.guid, qtyTotal: this.model.qtyTotal, qtyALive: this.model.qtyALive}).subscribe(
          (res) => {
            if (res.success === true) {
              this.alertify.success(this.alert.updated_ok_msg);
              this.refresh();
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        if (this.model.breedingGuid && this.model.sowGuid) {
           this.loadSelectedPig();
        }
        this.getAudit(this.model.id);
        this.title = "Sow_Farrow_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Sow_Farrow_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
