...
import { Breeding2SowConst, SowPregnancyTest } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SowService, BreedingService, SowPregnancyTestService } from "../../services";
declare let window: any;
@Component({
  selector: "app-sow-pregnancy-test",
  templateUrl: "./sow-pregnancy-test.component.html",
  styleUrls: ["./sow-pregnancy-test.component.scss"],
})
export class SowPregnancyTestComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSowPregnancyTest = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SowPregnancyTestService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SowPregnancyTest)
      .toPromise();
  }
 ...
 async loadSelectedPig() {
    ...
    const res = await this.service.getBreeding2SowInForSowPregnancyTest(p).toPromise()
      this.selectedPigData = res as any;
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SowPregnancyTest
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSowPregnancyTest(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SowPregnancyTest);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SowPregnancyTest/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: SowPregnancyTest) {
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Sow_PregnancyTest_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Sow_PregnancyTest_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
