...
import { BreedingSowPregnancyTestService } from '../../services';
import { BreedingSowPregnancyTest } from '../../config';

declare let window:any;

@Component({
  selector: 'app-breeding-sow-pregnancy-test',
  templateUrl: './breeding-sow-pregnancy-test.component.html',
  styleUrls: ['./breeding-sow-pregnancy-test.component.scss']
})
export class BreedingSowPregnancyTestComponent extends BaseComponent implements OnInit {
  isAdmin = JSON.parse(localStorage.getItem('user'))?.groupCode === 'ADMIN_CANCEL';
  data: DataManager;
  modalReference: NgbModalRef;

  @ViewChild('grid') public grid: GridComponent;
  model: BreedingSowPregnancyTest;
  ...
  constructor(
    private service: BreedingSowPregnancyTestService,
  ...
  loadLang() {
    this.translate.get('BreedingSowPregnancyTest').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BreedingSowPregnancyTest/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.model.farmGuid = localStorage.getItem('farmGuid');
        this.service.update(this.model as BreedingSowPregnancyTest).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as BreedingSowPregnancyTest) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'BreedingSowPregnancyTest_Edit_Modal';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'BreedingSowPregnancyTest_Add_Modal';
    }
