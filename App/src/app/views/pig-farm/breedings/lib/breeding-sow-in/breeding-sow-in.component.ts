...
@Component({
  selector: "app-breeding-sow-in-list",
  templateUrl: "./breeding-sow-in.component.html",
  styleUrls: ["./breeding-sow-in.component.scss"],
})
export class BreedingSowInComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectBreeding2SowIn = new EventEmitter();
  data: DataManager;
  ...
  recordClick(args: any) {
    ...
    this.service.changeBreeding2SowIn(args.rowData);
  }
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.model.id = 0;
      this.model.breedingType = "mating";
  ...
  async loadPig() {
    this.dataPig = await this.servicePig.getPigByPen(this.penGuid).toPromise();
  }
  onChangeBreeding(e) {
    if (e.isInteracted) {
      this.penGuid = e.itemData?.roomGuid;
    }
  }
}
