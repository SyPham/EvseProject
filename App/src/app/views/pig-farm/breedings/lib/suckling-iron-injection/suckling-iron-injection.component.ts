...
import { Breeding2SowConst, Breeding2Suckling, SucklingIronInjection } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SucklingService, BreedingService, SucklingIronInjectionService } from "../../services";
declare let window: any;
@Component({
  selector: "app-suckling-iron-injection",
  templateUrl: "./suckling-iron-injection.component.html",
  styleUrls: ["./suckling-iron-injection.component.css"],
})
export class SucklingIronInjectionComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSucklingIronInjection = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SucklingIronInjectionService,
    private serviceBreeding: BreedingService,
    private serviceBreeding2Suckling: Breeding2SucklingService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SucklingIronInjection)
      .toPromise();
  }
  ...
  async onChangeSowGuid(e) {
    if (e.isInteracted) {
      this.model.sowGuid = e.itemData?.guid;
      await this.loadBreeding2SucklingData();
    }
  }
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SucklingIronInjection
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSucklingIronInjection(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SucklingIronInjection);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SucklingIronInjection/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  create() {
    this.alertify.confirm4(
      ...
      () => {
        this.serviceBreeding2Suckling.endEditChange(true)
        const farmGuid = localStorage.getItem("farmGuid");
        if (!farmGuid) {
        ...
        this.model.breeding2Suckling = this.selectedPigData;
  ...
  setStatus(model: SucklingIronInjection) {
  ...
  async loadBreeding2SucklingData() {
    if (this.model?.id > 0) {
      this.serviceBreeding2Suckling.getBreeding2SucklingV4(this.model.breedingGuid, localStorage.getItem('lang'),this.model.sowGuid, this.model.guid).subscribe((res: any) => {
        this.selectedPigData = res;
      });
      //this.selectedPigData = await this.serviceBreeding2Suckling.getBreeding2SucklingV3(this.model.breedingGuid, this.model.sowGuid, this.model.guid, Breeding2SowConst.SucklingIronInjection).toPromise();
      if(this.selectedPigData.length === 0) {
       this.addSuckling()
      }
    } else {
      this.addSuckling()
  ...
  addSuckling() {
    let item = {} as Breeding2Suckling;
    item.breedingGuid = this.model.breedingGuid
    item.sowGuid = this.model.sowGuid
    this.selectedPigData= []
    this.selectedPigData.push(item)
  }
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Suckling_IronInjection_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Suckling_IronInjection_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
