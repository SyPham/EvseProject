...
import { Breeding2GiltInComponent } from './breeding2-gilt-in/breeding2-gilt-in.component';
import { Breeding2GiltInService, GiltInService } from '../../services';
import { Breeding2GiltIn, GiltIn } from '../../config';
declare let window: any;
@Component({
  selector: 'app-gilt-in',
  templateUrl: './gilt-in.component.html',
  styleUrls: ['./gilt-in.component.css']
})
export class GiltInComponent
  extends BaseComponent
  implements OnInit, OnDestroy, AfterViewInit
  {
    public query: Query;
    localLang = window.navigator.userLanguage || window.navigator.language;
    @Output() selectGiltIn = new EventEmitter();
    data: DataManager;
  ...
  realTotalWeight: any;
  realQty: any;
  @ViewChild(Breeding2GiltInComponent, {static: false}) breeding2GiltInComponent: Breeding2GiltInComponent
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "GiltIn")
        .toPromise();
    }
    async loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "GiltIn");
  ...
  // life cycle ejs-grid
    ...
    recordClick(args: any) {
      ...
      this.service.changeGiltIn(args.rowData);
    }
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}GiltIn/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    update() {
      ...
      // this.breeding2GiltInService.updateWeight(params).subscribe(
      //   (res) => {
      //     if (res.success === true) {
      //       this.alertify.success(this.alert.updated_ok_msg);
      //     } else {
