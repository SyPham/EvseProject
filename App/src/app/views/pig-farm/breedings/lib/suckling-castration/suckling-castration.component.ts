...
import { Breeding2SowConst, Breeding2Suckling, SucklingCastration } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SucklingService, BreedingService, SucklingCastrationService } from "../../services";
declare let window: any;
@Component({
  selector: "app-suckling-castration",
  templateUrl: "./suckling-castration.component.html",
  styleUrls: ["./suckling-castration.component.css"],
})
export class SucklingCastrationComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSucklingCastration = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SucklingCastrationService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SucklingCastration)
      .toPromise();
  }
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SucklingCastration
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSucklingCastration(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SucklingCastration);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SucklingCastration/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: SucklingCastration) {
  ...
  async loadBreeding2SucklingData() {
    if (this.model?.id > 0) {
      ...
      //this.selectedPigData = await this.serviceBreeding2Suckling.getBreeding2SucklingV3(this.model.breedingGuid, this.model.sowGuid, this.model.guid, Breeding2SowConst.SucklingCastration).toPromise();
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Suckling_Castration_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Suckling_Castration_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
