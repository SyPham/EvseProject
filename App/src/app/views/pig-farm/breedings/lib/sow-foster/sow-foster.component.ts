...
import { Breeding2SowConst, SowFoster } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SowService, Breeding2SucklingService, BreedingService, SowFosterService } from "../../services";
declare let window: any;
@Component({
  selector: "app-sow-foster",
  templateUrl: "./sow-foster.component.html",
  styleUrls: ["./sow-foster.component.scss"],
})
export class SowFosterComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSowFoster = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SowFosterService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SowFoster)
      .toPromise();
  }
  ...
  recordClick(args: any) {
    ...
    this.service.changeSowFoster(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SowFoster);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SowFoster/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Sow_Foster_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Sow_Foster_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
