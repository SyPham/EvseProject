...
import { Breeding2SowConst, Breeding2Suckling, SucklingWeaning } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SucklingService, BreedingService, SucklingWeaningService } from "../../services";
declare let window: any;
@Component({
  selector: "app-suckling-weaning",
  templateUrl: "./suckling-weaning.component.html",
  styleUrls: ["./suckling-weaning.component.css"],
})
export class SucklingWeaningComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSucklingWeaning = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SucklingWeaningService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SucklingWeaning)
      .toPromise();
  }
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SucklingWeaning
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSucklingWeaning(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SucklingWeaning);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SucklingWeaning/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: SucklingWeaning) {
  ...
  async loadBreeding2SucklingData() {
    if (this.model?.id > 0) {
      this.serviceBreeding2Suckling.getBreeding2SucklingWeaning(this.model.breedingGuid, localStorage.getItem('lang'),this.model.sowGuid).subscribe((res: any) => {
        this.selectedPigData = res;
      });
      //this.selectedPigData = await this.serviceBreeding2Suckling.getBreeding2SucklingV3(this.model.breedingGuid, this.model.sowGuid, this.model.guid, Breeding2SowConst.SucklingWeaning).toPromise();
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Suckling_Weaning_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Suckling_Weaning_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
