...
import { BreedingSowPickService } from '../../services';
import { BreedingSowPick } from '../../config';

declare let window:any;

@Component({
  selector: 'app-breeding-sow-pick',
  templateUrl: './breeding-sow-pick.component.html',
  styleUrls: ['./breeding-sow-pick.component.scss']
})
export class BreedingSowPickComponent extends BaseComponent implements OnInit {
  isAdmin = JSON.parse(localStorage.getItem('user'))?.groupCode === 'ADMIN_CANCEL';
  data: DataManager;
  modalReference: NgbModalRef;

  @ViewChild('grid') public grid: GridComponent;
  model: BreedingSowPick;
  ...
  constructor(
    private service: BreedingSowPickService,
  ...
  loadLang() {
    this.translate.get('BreedingSowPick').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BreedingSowPick/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.model.farmGuid = localStorage.getItem('farmGuid');
        this.service.update(this.model as BreedingSowPick).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as BreedingSowPick) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'BreedingSowPick_Edit_Modal';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'BreedingSowPick_Add_Modal';
    }
