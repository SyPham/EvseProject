...
import { Breeding2SowConst, Breeding2Suckling, SucklingMove } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SucklingService, BreedingService, SucklingMoveService } from "../../services";
declare let window: any;
@Component({
  selector: "app-suckling-move",
  templateUrl: "./suckling-move.component.html",
  styleUrls: ["./suckling-move.component.css"],
})
export class SucklingMoveComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSucklingMove = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SucklingMoveService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SucklingMove)
      .toPromise();
  }
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SucklingMove
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSucklingMove(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SucklingMove);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SucklingMove/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  async loadBreeding2SucklingData() {
    if (this.model?.id > 0) {
      ...
      //this.selectedPigData = await this.serviceBreeding2Suckling.getBreeding2SucklingV3(this.model.breedingGuid, this.model.sowGuid, this.model.guid, Breeding2SowConst.SucklingMove).toPromise();
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Suckling_Move_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Suckling_Move_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
