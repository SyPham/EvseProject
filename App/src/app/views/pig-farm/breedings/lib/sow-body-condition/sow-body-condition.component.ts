...
import { Breeding2SowConst, SowBodyCondition } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SowService, BreedingService, SowBodyConditionService } from "../../services";
declare let window: any;
@Component({
  selector: "app-sow-body-condition",
  templateUrl: "./sow-body-condition.component.html",
  styleUrls: ["./sow-body-condition.component.scss"],
})
export class SowBodyConditionComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSowBodyCondition = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SowBodyConditionService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SowBodyCondition)
      .toPromise();
  }
  ...
  async loadSelectedPig() {
    ...
    const p = {
      ...
      type: Breeding2SowConst.SowBodyCondition,
      lang: this.locale
    };
    const res = await this.servicePig.getSelectedSow(p).toPromise()
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SowBodyCondition
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSowBodyCondition(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SowBodyCondition);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SowBodyCondition/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  create(giltIn) {
    this.alertify.confirm4(
      ...
      () => {
        ...
        this.model.breeding2Sow = giltIn.dataSource;
  ...
  setStatus(model: SowBodyCondition) {
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Sow_BodyCondition_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Sow_BodyCondition_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
