...
@Component({
  ...
  templateUrl: './bom-weighing.component.html',
  styleUrls: ['./bom-weighing.component.scss']
})
export class BomWeighingComponent  extends BaseComponent implements OnInit {
  @Input() bomSow: BomGilt;
  data: DataManager;
  ...
  constructor(
    ...
    private serviceShare: BomGiltService,
  ...
  ngOnInit() {
    ...
    L10n.load(load);
    this.subscription = this.serviceShare.currentBomGilt.subscribe(bomSow => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BOMWeighing/LoadData?bomGuid=${this.bomSow?.guid}&lang=${this.globalLang}`,
