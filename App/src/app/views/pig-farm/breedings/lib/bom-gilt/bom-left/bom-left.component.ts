...
export class BomLeftComponent extends BaseComponent implements OnInit {
  @Output() selectBomGilt = new EventEmitter();
  data: DataManager;
  password = '';
  ...
  ngOnInit() {
    ...
    L10n.load(load);
    this.service.changeBomGilt({} as BomGilt);
    this.loadData();
  }
  ...
  rowSelected(args) {
    if (args.data) {
      this.service.changeBomGilt(args.data);
    }
  }
  recordClick(args: any) {
    //console.log(args.rowData);
    this.service.changeBomGilt(args.rowData);
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BomGilt/LoadData?lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as BomGilt) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'BomGilt_Edit_Modal';
    } else {
      this.model.id = 0;
      this.title = 'BomGilt_Add_Modal';
    }
