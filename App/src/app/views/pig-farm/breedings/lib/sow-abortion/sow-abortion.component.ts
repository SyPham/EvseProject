...
import { Breeding2SowConst, SowAbortion } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SowService, BreedingService, SowAbortionService } from "../../services";
declare let window: any;
@Component({
  selector: "app-sow-abortion",
  templateUrl: "./sow-abortion.component.html",
  styleUrls: ["./sow-abortion.component.scss"],
})
export class SowAbortionComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSowAbortion = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SowAbortionService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SowAbortion)
      .toPromise();
  }
  ...
  loadSelectedPig() {
    ...
    const p = {
      ...
      type: Breeding2SowConst.SowAbortion,
      lang: this.locale
    };
  ...
  recordClick(args: any) {
    ...
    this.service.changeSowAbortion(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SowAbortion);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SowAbortion/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  execute(breeding2sow) {
    breeding2sow.grid.endEdit();
    if (this.model.id == 0) {
      this.create();
    } else {
  ...
  async loadBreeding2SowData() {
   ...
   loadBreeding2SowDataForEdit() {
    let query = new Query()
    .where("recordGuid", "equal", this.model.guid)
    .addParams("lang", localStorage.getItem('lang'));
    let data = new DataManager({
      url: `${this.baseUrl}Breeding2Sow/LoadData`,
      adaptor: new UrlAdaptor,
      crossDomain: true,
    });
    data.executeQuery(query).then((x: any)=> {
      console.log(x.result)
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.loadBreeding2SowDataForEdit();
        // if (this.model.breedingGuid) {
        //   await this.loadBreeding2SowData();
        // }
        this.getAudit(this.model.id);
        this.title = "Sow_Abortion_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Sow_Abortion_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
