...
import { Breeding2SowConst, Breeding2Suckling, SucklingTeachingSlot } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SucklingService, BreedingService, SucklingTeachingSlotService } from "../../services";
declare let window: any;
@Component({
  selector: "app-suckling-teaching",
  templateUrl: "./suckling-teaching.component.html",
  styleUrls: ["./suckling-teaching.component.css"],
})
export class SucklingTeachingComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSucklingTeaching = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SucklingTeachingSlotService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SucklingTeachingSlot)
      .toPromise();
  }
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SucklingTeachingSlot
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSucklingTeachingSlot(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SucklingTeachingSlot);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SucklingTeachingSlot/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: SucklingTeachingSlot) {
  ...
  async loadBreeding2SucklingData() {
    if (this.model?.id > 0) {
      ...
      //this.selectedPigData = await this.serviceBreeding2Suckling.getBreeding2SucklingV3(this.model.breedingGuid, this.model.sowGuid, this.model.guid, Breeding2SowConst.SucklingTeachingSlot).toPromise();
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "TeachingSlot_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "TeachingSlot_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
