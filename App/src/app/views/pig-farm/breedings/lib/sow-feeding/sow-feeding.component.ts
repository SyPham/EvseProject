...
import { SowFeedingService } from "../../services/new/sow-feeding.service";
declare let window: any;
@Component({
  selector: 'pigfarm-sow-feeding',
  templateUrl: './sow-feeding.component.html',
  styleUrls: ['./sow-feeding.component.scss']
})
export class SowFeedingComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSowFeeding = new EventEmitter();
  data: DataManager;
...
disabledInput(date : any) {
  ...
  constructor(
    private service: SowFeedingService,
...
this.loadRoom();
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Sow_Feeding")
      .toPromise();
  }
  loadPen() {
...
// life cycle ejs-grid
  ...
  recordClick(args: any) {
    ...
    this.service.changeSowFeeding(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Sow_Feeding");
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SowFeeding/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  onParamChange() {
    this.route.paramMap.subscribe((params: any) => {
      ...
      if (upperguid) {
        this.service.getBySowGuid(upperguid,recordguid)
        .subscribe((model: any) => {
          this.model = {...model};
  ...
  toggleRecordDate(id) {
    this.alertify.confirm4(
      ...
      this.alert.updateMessage,
      () => {
        this.service.toggleSowDate(id).subscribe(
          (res) => {
            if (res.success === true) {
              this.alertify.success(this.alert.updated_ok_msg);
              this.loadData();
