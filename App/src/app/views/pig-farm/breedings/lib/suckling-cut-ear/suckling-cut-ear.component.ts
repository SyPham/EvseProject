...
import { Breeding2SowConst, Breeding2Suckling, SucklingCutEar } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SucklingService, BreedingService, SucklingCutEarService } from "../../services";
declare let window: any;
@Component({
  selector: "app-suckling-cut-ear",
  templateUrl: "./suckling-cut-ear.component.html",
  styleUrls: ["./suckling-cut-ear.component.css"],
})
export class SucklingCutEarComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSucklingCutEar = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SucklingCutEarService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SucklingCutEar)
      .toPromise();
  }
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SucklingCutEar
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSucklingCutEar(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SucklingCutEar);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SucklingCutEar/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: SucklingCutEar) {
  ...
  async loadBreeding2SucklingData() {
    if (this.model?.id > 0) {
      ...
      //this.selectedPigData = await this.serviceBreeding2Suckling.getBreeding2SucklingV3(this.model.breedingGuid, this.model.sowGuid, this.model.guid, Breeding2SowConst.SucklingCutEar).toPromise();
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Suckling_CutEar_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Suckling_CutEar_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
