...
import { Breeding2SowConst, SowHeating } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SowService, BreedingService, SowHeatingService } from "../../services";
declare let window: any;
@Component({
  selector: "app-sow-heating",
  templateUrl: "./sow-heating.component.html",
  styleUrls: ["./sow-heating.component.scss"],
})
export class SowHeatingComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSowHeating = new EventEmitter();
  data: DataManager;
  ...
  public onFilteringPig: any = (e: any) => {
    ...
    e.updateData(
      this.dataPig,
      e.text !== "" ? new Query().search(e.text, ["name"]) : new Query()
    );
  };
  ...
  onChangePig(e) {
    if (e.isInteracted) {
        // this.loadBreeding2SowIn();
        this.selectedPigs = this.dataPig.filter(x=> this.selectedPig.includes(x.guid))
    }
  }
  constructor(
    private service: SowHeatingService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SowHeating)
      .toPromise();
  }

  async onChangebreedingGuid(e) {
    this.roomGuid = e.itemData?.roomGuid
    if (e.itemData?.penGuid && this.selectedPen) {
      this.selectedPen.push(e.itemData?.penGuid)
    } else {
      this.selectedPen =[]
      this.selectedPen.push(e.itemData?.penGuid)
    }
  ...
  async loadSelectedPig() {
    ...
    const p = {
      ...
      type: Breeding2SowConst.SowHeating,
      lang: this.locale,
    };
  ...
  loadBreeding2SowIn() {
    ...
    new DataManager({
      url: `${this.baseUrl}Breeding2SowIn/GetDataDropdownlist?lang=${localStorage.getItem("lang")}`,
  ...
  recordClick(args: any) {
    ...
    this.service.changeSowHeating(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SowHeating);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SowHeating/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  ToFormatModel(model: any) {
    for (let key in model) {
      ...
      if (value && value instanceof Date) {
        if (key === "createDate" || key === 'matingDate' || key === 'heatingDate') {
          model[key] = this.datePipe.transform(value, "yyyy-MM-dd HH:mm:ss");
        } else {
          model[key] = this.datePipe.transform(value, "yyyy-MM-dd");
        }
      } else if (value && key === 'matingDate' || key === 'heatingDate') {
        model[key] = this.datePipe.transform(value, "yyyy-MM-dd");
      }
      else {
        model[key] = value;
  ...
  create() {
    this.alertify.confirm4(
      ...
      () => {
        ...
        this.model.breeding2Sow = this.selectedPigData.map(item => {return {...this.ToFormatModel(item)}});
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Sow_Heating_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Sow_Heating_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
