...
@Component({
  selector: 'app-breeding-sow-in-detail',
  templateUrl: './breeding-sow-in.component.html',
  styleUrls: ['./breeding-sow-in.component.scss']
})
export class BreedingSowInDetailComponent  extends BaseComponent implements OnInit {
  ...
  openModal(template, data = {} as BreedingSowHeating) {
    if (data?.id > 0) {
      ...
      this.title = 'BreedingSowIn_Edit_Modal';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'BreedingSowIn_Add_Modal';
    }
