...
@Component({
  selector: 'app-bom-feeding',
  templateUrl: './bom-feeding.component.html',
  styleUrls: ['./bom-feeding.component.scss']
})
export class BomFeedingComponent extends BaseComponent implements OnInit {
  @Input() bomBoar: BomBoar;
  data: DataManager;
  ...
  constructor(
    ...
    private serviceShare: BomBoarService,
  ...
  ngOnInit() {
    ...
    L10n.load(load);
    this.subscription = this.serviceShare.currentBomBoar.subscribe(bomBoar => {
      this.bomBoar = bomBoar as any
      if (this.bomBoar?.guid) {
        this.loadData();
      }
    });
  }
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BOMFeeding/LoadDataByBomGuid?bomGuid=${this.bomBoar?.guid}&lang=${this.globalLang}`,
  ...
  create() {
   ...
   this.alertify.confirm4(
      ...
      () => {
        this.model.bomGuid = this.bomBoar?.guid;
