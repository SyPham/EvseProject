...
export class ImmunizationComponent extends BaseComponent implements OnInit {
  ...
  openModal(template, data = {} as any) {
    if (!this.bomSow?.guid) {
      this.alertify.warning(this.translate.instant('Please choose the BomBoar!'), true);
      return;
    }
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'BomBoar_IMMUNIZATION_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'BomBoar_IMMUNIZATION_ADD_MODAL';
    }
