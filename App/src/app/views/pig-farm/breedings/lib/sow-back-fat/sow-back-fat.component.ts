...
import { SowBackFatService } from "../../services/new/sow-back-fat.service";
import { Breeding2GiltInService } from "../../services";
declare let window: any;
@Component({
  selector: "pigfarm-sow-back-fat",
  templateUrl: "./sow-back-fat.component.html",
  styleUrls: ["./sow-back-fat.component.scss"],
})
export class SowBackFatComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSowBackFat = new EventEmitter();
  data: DataManager;
  ...
  public commands: CommandModel[] = [
    {
      ...
      buttonOption: {
        iconCss: "fa fa-trash ",
        cssClass: "delete-grid e-small",
      },
    },
    {
      type: "Edit",
      buttonOption: { iconCss: "fa fa-edit", cssClass: "edit-grid e-small" },
    },
  ];
  breeding2GiltInsData: any;
  realTotalWeight: any;
  realQty: any;
  giltInGuid: any;
  ...
  constructor(
    private service: SowBackFatService,
    ...
    private breeding2GiltInService: Breeding2GiltInService
  ) {
  ...
  ngAfterViewInit(): void {
    this.subscription.add(
      this.breeding2GiltInService.currentbreeding2GiltIn.subscribe((x) => {
    ...
    this.subscription.add(
      this.breeding2GiltInService.realTotalWeightCurrent.subscribe((x) => {
        if (x) {
    ...
    this.subscription.add(
      this.breeding2GiltInService.realQtyCurrent.subscribe((x) => {
        if (x) {
  ...
  commandClickLink(e) {
    const type = e.commandColumn.type;
    if (type === "Edit") {
      this.model = {} as any;
      this.service.getById(e.rowData.id).subscribe((x) => {
        this.openModal(this.templateRef, x);
      });
    }
    if (type === "Delete") {
      this.delete(e.rowData.id);
    }
  }
  loadRoom() {
    this.dataRoom = this.serviceRoom.getRoomsByFarmGuid(localStorage.getItem('farmGuid'), "", '');
  }
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Sow_BackFat")
      .toPromise();
  }
  async loadPen() {
  ...
  onChangeMakeOrder(e) {
    ...
    this.giltInGuid = e.itemData?.giltInGuid;
  }
  onChangeRoom(e) {
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Sow_BackFat");
  ...
  recordClick(args: any) {
    ...
    this.service.changeSowBackFat(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Sow_BackFat");
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SowBackFat/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  getCheckedPigData() {
    this.servicePig
      .getSelectedPigsByRecord(this.model?.guid, "Sow_BackFat")
      .subscribe((data: any) => {
  ...
  create() {
    this.alertify.confirm4(
      ...
      () => {
        ...
        this.model.record2Pigs = this.breeding2GiltInsData;
  ...
  async openModal(template, data = {} as any) {
    ...
    this.giltInGuid = null;
    this.breeding2GiltInsData = [];
    this.pigData = [];
    if (data?.id > 0) {
      this.model = { ...data };
      try {
      ...
      this.spinner.hide("default");
      this.title = "Sow_BackFat_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.title = "Sow_BackFat_ADD_MODAL";
    }
