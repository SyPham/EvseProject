...
import { Breeding2SucklingService, BreedingService, SucklingWeighingService } from "../../services";
import { Breeding2SowConst, Breeding2Suckling, SucklingWeighing } from "../../config";
declare let window: any;
@Component({
  selector: "app-suckling-weighing",
  templateUrl: "./suckling-weighing.component.html",
  styleUrls: ["./suckling-weighing.component.css"],
})
export class SucklingWeighingComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSucklingWeighing = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SucklingWeighingService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SucklingWeighing)
      .toPromise();
  }
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SucklingWeighing
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSucklingWeighing(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SucklingWeighing);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SucklingWeighing/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: SucklingWeighing) {
  ...
  async loadBreeding2SucklingData() {
    if (this.model?.id > 0) {
      ...
      //this.selectedPigData = await this.serviceBreeding2Suckling.getBreeding2SucklingV3(this.model.breedingGuid, this.model.sowGuid, this.model.guid, Breeding2SowConst.SucklingWeighing).toPromise();
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Suckling_Weighing_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Suckling_Weighing_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
