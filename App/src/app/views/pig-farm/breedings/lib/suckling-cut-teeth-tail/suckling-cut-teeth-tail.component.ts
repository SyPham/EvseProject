...
import { Breeding2SowConst, Breeding2Suckling, SucklingCutTeethTail } from "../../config";
import { Record2PigService } from "src/app/_core/_service/apply-orders";
import { Breeding2SucklingService, BreedingService, SucklingCutTeethTailService } from "../../services";
declare let window: any;
@Component({
  selector: "app-suckling-cut-teeth-tail",
  templateUrl: "./suckling-cut-teeth-tail.component.html",
  styleUrls: ["./suckling-cut-teeth-tail.component.css"],
})
export class SucklingCutTeethTailComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectSucklingCutTeethTail = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: SucklingCutTeethTailService,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", Breeding2SowConst.SucklingCutTeethTail)
      .toPromise();
  }
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate(
      "type",
      "equal",
      Breeding2SowConst.SucklingCutTeethTail
    );
  ...
  recordClick(args: any) {
    ...
    this.service.changeSucklingCutTeethTail(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", Breeding2SowConst.SucklingCutTeethTail);
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}SucklingCutTeethTail/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: SucklingCutTeethTail) {
  ...
  async loadBreeding2SucklingData() {
    if (this.model?.id > 0) {
     ...
     // this.selectedPigData = await this.serviceBreeding2Suckling.getBreeding2SucklingV3(this.model.breedingGuid, this.model.sowGuid, this.model.guid, Breeding2SowConst.SucklingCutTeethTail).toPromise();
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      try {
        ...
        this.title = "Suckling_CutTeethTail_EDIT_MODAL";
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "Suckling_CutTeethTail_ADD_MODAL";
      this.showModal(template);
    }
  }
  showModal(template) {
