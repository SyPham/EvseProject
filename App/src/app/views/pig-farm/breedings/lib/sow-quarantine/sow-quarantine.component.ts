...
import { SowQuarantineService } from '../../services/new/sow-quarantine.service';
declare let window: any;
@Component({
  selector: 'pigfarm-sow-quarantine',
  templateUrl: './sow-quarantine.component.html',
  styleUrls: ['./sow-quarantine.component.scss']
})
export class SowQuarantineComponent 
  extends BaseComponent
  implements OnInit, OnDestroy
  {
    public query: Query;
    localLang = window.navigator.userLanguage || window.navigator.language;
    @Output() selectSowQuarantine = new EventEmitter();
    data: DataManager;
...
public onFilteringPig: any = (e: any) => {
  ...
  disabledInput(date : any) {
    ...
    constructor(
      private service: SowQuarantineService,
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Sow_Quarantine")
        .toPromise();
    }
    async loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Sow_Quarantine");
  ...
  // life cycle ejs-grid
    ...
    recordClick(args: any) {
      ...
      this.service.changeSowQuarantine(args.rowData);
    }
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Sow_Quarantine");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}SowQuarantine/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        this.spinner.hide('default');
        this.title = "Sow_Quarantine_EDIT_MODAL";
      } else {
        this.model.id = 0;
        ...
        this.title = "Sow_Quarantine_ADD_MODAL";
      }
