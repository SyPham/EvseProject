...
import { BreedingSowHeatingService } from '../../services';
import { BreedingSowHeating } from '../../config';

declare let window:any;

@Component({
  selector: 'app-breeding-sow-heating',
  templateUrl: './breeding-sow-heating.component.html',
  styleUrls: ['./breeding-sow-heating.component.scss']
})
export class BreedingSowHeatingComponent extends BaseComponent implements OnInit {
  isAdmin = JSON.parse(localStorage.getItem('user'))?.groupCode === 'ADMIN_CANCEL';
  data: DataManager;
  modalReference: NgbModalRef;

  @ViewChild('grid') public grid: GridComponent;
  model: BreedingSowHeating;
  ...
  constructor(
    private service: BreedingSowHeatingService,
  ...
  loadLang() {
    this.translate.get('BreedingSowHeating').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BreedingSowHeating/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.model.farmGuid = localStorage.getItem('farmGuid');
        this.service.update(this.model as BreedingSowHeating).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as BreedingSowHeating) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'BreedingSowHeating_Edit_Modal';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'BreedingSowHeating_Add_Modal';
    }
