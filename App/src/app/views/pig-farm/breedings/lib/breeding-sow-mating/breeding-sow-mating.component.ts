...
import { BreedingSowMatingService } from '../../services';
import { BreedingSowMating } from '../../config';

declare let window:any;

@Component({
  selector: 'app-breeding-sow-mating',
  templateUrl: './breeding-sow-mating.component.html',
  styleUrls: ['./breeding-sow-mating.component.scss']
})
export class BreedingSowMatingComponent extends BaseComponent implements OnInit {
  isAdmin = JSON.parse(localStorage.getItem('user'))?.groupCode === 'ADMIN_CANCEL';
  data: DataManager;
  modalReference: NgbModalRef;

  @ViewChild('grid') public grid: GridComponent;
  model: BreedingSowMating;
  ...
  constructor(
    private service: BreedingSowMatingService,
  ...
  loadLang() {
    this.translate.get('BreedingSowMating').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BreedingSowMating/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.model.farmGuid = localStorage.getItem('farmGuid');
        this.service.update(this.model as BreedingSowMating).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as BreedingSowMating) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'BreedingSowMating_Edit_Modal';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'BreedingSowMating_Add_Modal';
    }
