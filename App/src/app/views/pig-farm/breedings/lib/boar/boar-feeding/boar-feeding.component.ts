...
import { BoarFeedingService } from "../../../services/new/boar-feeding.service";
declare let window: any;
@Component({
  selector: 'pigfarm-boar-feeding',
  templateUrl: './boar-feeding.component.html',
  styleUrls: ['./boar-feeding.component.css']
})
export class BoarFeedingComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectBoarFeeding = new EventEmitter();
  data: DataManager;
...
disabledInput(date : any) {
  ...
  constructor(
    private service: BoarFeedingService,
...
pigGuid: string;
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Boar_Feeding")
      .toPromise();
  }
  loadPen() {
...
// life cycle ejs-grid
  ...
  recordClick(args: any) {
    ...
    this.service.changeBoarFeeding(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Boar_Feeding");
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}BoarFeeding/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  onParamChange() {
    this.route.paramMap.subscribe((params: any) => {
      ...
      if (upperguid) {
        this.service.getByBoarGuid(upperguid,recordguid)
        .subscribe((model: any) => {
          this.model = {...model};
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = "Boar_Feeding_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.title = "Boar_Feeding_ADD_MODAL";
    }
  ...
  toggleRecordDate(id) {
    this.alertify.confirm4(
      ...
      this.alert.updateMessage,
      () => {
        this.service.toggleBoarDate(id).subscribe(
          (res) => {
            if (res.success === true) {
              this.alertify.success(this.alert.updated_ok_msg);
              this.loadData();
