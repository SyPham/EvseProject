...
import { BoarSemenMixService } from '../../../services/new/boar-semen-mix.service';
import { Record2PigService } from 'src/app/_core/_service/apply-orders';
declare let window: any;

@Component({
  selector: 'pigfarm-boar-semen-mix',
  templateUrl: './boar-semen-mix.component.html',
  styleUrls: ['./boar-semen-mix.component.css']
})
export class BoarSemenMixComponent
  extends BaseComponent
  implements OnInit, OnDestroy
  {
    public query: Query;
    localLang = window.navigator.userLanguage || window.navigator.language;
    @Output() selectBoarSemenMix = new EventEmitter();
    data: DataManager;
...
public onFilteringPig: any = (e: any) => {
  ...
  disabledInput(date : any) {
    ...
    constructor(
      private service: BoarSemenMixService,
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Boar_SemenMix")
        .toPromise();
    }
    async loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Boar_SemenMix");
  ...
  // life cycle ejs-grid
    ...
    recordClick(args: any) {
      ...
      this.service.changeBoarSemenMix(args.rowData);
    }
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Boar_SemenMix");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}BoarSemenMix/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        this.spinner.hide('default');
        this.title = "BoarSemenMix_EDIT_MODAL";
      } else {
        this.model.id = 0;
        ...
        this.title = "BoarSemenMix_ADD_MODAL";
      }
