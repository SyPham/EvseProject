...
import { BoarQuarantineService } from '../../../services/new/boar-quarantine.service';
declare let window: any;
@Component({
  selector: 'app-boar-quarantine',
  templateUrl: './boar-quarantine.component.html',
  styleUrls: ['./boar-quarantine.component.css']
})
export class BoarQuarantineComponent
  extends BaseComponent
  implements OnInit, OnDestroy
  {
    public query: Query;
    localLang = window.navigator.userLanguage || window.navigator.language;
    @Output() selectBoarQuarantine = new EventEmitter();
    data: DataManager;
...
public onFilteringPig: any = (e: any) => {
  ...
  disabledInput(date : any) {
    ...
    constructor(
      private service: BoarQuarantineService,
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Boar_Quarantine")
        .toPromise();
    }
    async loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Boar_Quarantine");
  ...
  // life cycle ejs-grid
    ...
    recordClick(args: any) {
      ...
      this.service.changeBoarQuarantine(args.rowData);
    }
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Boar_Quarantine");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}BoarQuarantine/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    setStatus(model: BoarQuarantine) {
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        this.spinner.hide('default');
        this.title = "Boar_Quarantine_EDIT_MODAL";
      } else {
        this.model.id = 0;
        ...
        this.title = "Boar_Quarantine_ADD_MODAL";
      }
