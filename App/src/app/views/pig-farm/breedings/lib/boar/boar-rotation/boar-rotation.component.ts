...
import { BoarRotationService } from '../../../services/new/boar-rotation.service';
import { Record2PigService } from 'src/app/_core/_service/apply-orders';
declare let window: any;
@Component({
  selector: 'pigfarm-boar-rotation',
  templateUrl: './boar-rotation.component.html',
  styleUrls: ['./boar-rotation.component.css']
})
export class BoarRotationComponent
  extends BaseComponent
  implements OnInit, OnDestroy
  {
    public query: Query;
    localLang = window.navigator.userLanguage || window.navigator.language;
    @Output() selectBoarRotation = new EventEmitter();
    data: DataManager;
...
public onFilteringPig: any = (e: any) => {
  ...
  disabledInput(date : any) {
    ...
    constructor(
      private service: BoarRotationService,
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Boar_Rotation")
        .toPromise();
    }
    async loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Boar_Rotation");
  ...
  // life cycle ejs-grid
    ...
    recordClick(args: any) {
      ...
      this.service.changeBoarRotation(args.rowData);
    }
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Boar_Rotation");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}BoarRotation/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        this.spinner.hide('default');
        this.title = "Boar_Rotation_EDIT_MODAL";
      } else {
        this.model.id = 0;
        ...
        this.title = "Boar_Rotation_ADD_MODAL";
      }
