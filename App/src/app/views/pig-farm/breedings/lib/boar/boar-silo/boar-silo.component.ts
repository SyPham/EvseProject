...
import { BoarSiloService } from '../../../services/new/boar-silo.service';
import { Record2PigService } from 'src/app/_core/_service/apply-orders';

declare let window: any;
@Component({
  selector: 'pig-farm-boar-silo',
  templateUrl: './boar-silo.component.html',
  styleUrls: ['./boar-silo.component.scss']
})
export class BoarSiloComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectBoarSilo = new EventEmitter();
  data: DataManager;
  ...
  disabledInput(date : any) {
    ...
    constructor(
    private service: BoarSiloService,
...
private router: Router,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Boar_Silo")
      .toPromise();
  }
    onChangeMakeOrder(e) {
    if (e.isInteracted) {
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Boar_Silo");
  ...
  recordClick(args: any) {
    ...
    this.service.changeBoarSilo(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Boar_Silo");
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}BoarSilo/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
...
this.getAudit(this.model.id);
      this.title = "Boar_Silo_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.title = "Boar_Silo_ADD_MODAL";
    }
