...
// import { BoarTestingService } from '../../../services/new/boar-testing.service';
import { Record2PigService } from 'src/app/_core/_service/apply-orders';
import { BoarTestingService } from '../../../services/new/boar-testing.service';
declare let window: any;
@Component({
  selector: 'pigfarm-boar-testing',
  templateUrl: './boar-testing.component.html',
  styleUrls: ['./boar-testing.component.css']
})
export class BoarTestingComponent
  extends BaseComponent
  implements OnInit, OnDestroy
  {
    public query: Query;
    localLang = window.navigator.userLanguage || window.navigator.language;
    @Output() selectBoarTesting = new EventEmitter();
    data: DataManager;
...
public onFilteringPig: any = (e: any) => {
  ...
  disabledInput(date : any) {
    ...
    constructor(
      private service: BoarTestingService,
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Boar_Testing")
        .toPromise();
    }
    async loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Boar_Testing");
  ...
  // life cycle ejs-grid
    ...
    recordClick(args: any) {
      ...
      this.service.changeBoarTesting(args.rowData);
    }
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Boar_Testing");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}BoarTesting/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        this.spinner.hide('default');
        this.title = "Boar_Testing_EDIT_MODAL";
      } else {
        this.model.id = 0;
        ...
        this.title = "Boar_Testing_ADD_MODAL";
      }
