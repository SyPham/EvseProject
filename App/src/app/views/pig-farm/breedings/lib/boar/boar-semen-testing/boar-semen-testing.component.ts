...
import { BoarSemenTestingService } from '../../../services/new/boar-semen-testing.service';
declare let window: any;
@Component({
  selector: 'pigfarm-boar-semen-testing',
  templateUrl: './boar-semen-testing.component.html',
  styleUrls: ['./boar-semen-testing.component.css']
})
export class BoarSemenTestingComponent
  extends BaseComponent
  implements OnInit, OnDestroy
  {
    public query: Query;
    localLang = window.navigator.userLanguage || window.navigator.language;
    @Output() selectBoarSemenTesting = new EventEmitter();
    data: DataManager;
  ...
  public commands: CommandModel[] = [
    { type: 'Delete', buttonOption: { iconCss: 'fa fa-trash ', cssClass: 'delete-grid e-small' } },
    { type: 'Edit', buttonOption: { iconCss: 'fa fa-edit', cssClass: 'edit-grid e-small' } },
];
...
public onFilteringPig: any = (e: any) => {
  ...
  disabledInput(date : any) {
    ...
    constructor(
      private service: BoarSemenTestingService,
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Boar_SemenTesting")
        .toPromise();
    }
    async loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Boar_SemenTesting");
  ...
  // life cycle ejs-grid
    ...
    recordClick(args: any) {
      ...
      this.service.changeBoarSemenTesting(args.rowData);
    }
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Boar_SemenTesting");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}BoarSemenTesting/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    setStatus(model: BoarSemenTesting) {
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        this.spinner.hide('default');
        this.title = "Boar_SemenTesting_EDIT_MODAL";
      } else {
        this.model.id = 0;
        ...
        this.title = "Boar_SemenTesting_ADD_MODAL";
      }
