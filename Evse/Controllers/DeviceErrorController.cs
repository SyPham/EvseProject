...
{
    public class DeviceErrorController : ApiControllerBase
    {
        private readonly IDeviceErrorService _service;

        public DeviceErrorController(IDeviceErrorService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] DeviceErrorDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] DeviceErrorDto model)
        {
    ...
    [HttpPost]
        ...
        {
            var model = JsonConvert.DeserializeObject<DeviceErrorDto>(request);
            model.File = file;
            return Ok(await _service.AddFormAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateFormAsync([FromForm] DeviceErrorDto model)
        {
