...
namespace Evse.Controllers
{
    public class InvestorController : ApiControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] InvestorDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] InvestorDto model)
        {
            return StatusCodeResult(await _service.UpdateAsync(model));
        }

    [HttpPost]
        public async Task<ActionResult> AddFormAsync([FromForm] InvestorDto model)
        {
            return Ok(await _service.AddFormAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateFormAsync([FromForm] InvestorDto model)
        {
