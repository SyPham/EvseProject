...
{
    public class ElectricianController : ApiControllerBase
    {
        private readonly IElectricianService _service;

        public ElectricianController(IElectricianService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] ElectricianDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] ElectricianDto model)
        {
            return StatusCodeResult(await _service.UpdateAsync(model));
        }

    [HttpPost]
        public async Task<ActionResult> AddFormAsync([FromForm] ElectricianDto model)
        {
            return Ok(await _service.AddFormAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateFormAsync([FromForm] ElectricianDto model)
        {
