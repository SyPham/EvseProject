...
{
    public class ImageConfigController : ApiControllerBase
    {
        private readonly IImageConfigService _service;

        public ImageConfigController(IImageConfigService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] ImageConfigDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] ImageConfigDto model)
        {
            return StatusCodeResult(await _service.UpdateAsync(model));
        }

    [HttpPost]
        public async Task<ActionResult> AddFormAsync([FromForm] ImageConfigDto model)
        {
            return Ok(await _service.AddFormAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateFormAsync([FromForm] ImageConfigDto model)
        {
