...
{
    public interface IAuth2Service : IScopeService
    {
        Task<Account2> Login(string username, string password);
        Task LogOut();
    ...
    {
        private readonly IRepositoryBase<Account2> _repo;
        ...
        public async Task<Account2> Login(string username, string password)
        {
        ...
        {
            var account = await _repo.FindAll().FirstOrDefaultAsync(x => x.Id == ID && (x.Status == 1 || x.Status == 0));
        ...
        private async Task<OperationResult> GenerateOperationResultForUserAsync(Account2 user, string password)
        {
            var claims = new[]
            {
            ...
            //}
            //var xaccountGroup = await _repoAccount2Group.FindAll().AsNoTracking().FirstOrDefaultAsync(x => x.Guid == user.Account2Group && x.Status == 1);
...
private async Task<OperationResult> GenerateOperationResultForUserEngineerAsync(Engineer user, string password)
        ...
        private async Task<OperationResult> GenerateOperationResultForUserForGotPasswordAsync(Account2 user, string email)
        {
