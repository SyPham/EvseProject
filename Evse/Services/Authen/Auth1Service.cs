...
{
    public interface IAuth1Service : IScopeService
    {
    ...
    public class Auth1Service : IAuth1Service
    {
        ...
        private async Task<OperationResult> GenerateOperationResultForUserAsync(Account user, string password)
        {
            var claims = new[]
            {
            ...
            //}
            //var xaccountGroup = await _repoAccountGroup.FindAll().AsNoTracking().FirstOrDefaultAsync(x => x.Guid == user.AccountGroup && x.Status == 1);
...
private async Task<OperationResult> GenerateOperationResultForUserEngineerAsync(Engineer user, string password)
        ...
        private async Task<OperationResult> GenerateOperationResultForUserForGotPasswordAsync(Account user, string email)
        {
