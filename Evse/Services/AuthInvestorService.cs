...
{
    public interface IAuthInvestorService : IScopeService
    {
        Task<Investor> Login(string username, string password);
        Task LogOut();
    ...
    public class AuthInvestorService : IAuthInvestorService
    {
...
,
        ...
        public async Task<Investor> Login(string username, string password)
        {
        ...
        {
            string token = _httpContextAccessor.HttpContext.Request.Headers["AuthInvestororization"];
     ...
     public async  Task<OperationResult> ChangePassword(ChangePasswordDto reset) {
        ...
        private async Task<OperationResult> GenerateOperationResultForUserAsync(Investor user, string password)
        {
            var claims = new[]
            {
            ...
            await _unitOfWork.SaveChangeAsync();
            var userResponse = _mapper.Map<UserInvestorForDetailDto>(user);
             userResponse.Mobile = user.InvestorMobile;
            userResponse.Email = user.InvestorEmail;
            userResponse.FullName = user.InvestorName;
            userResponse.NickName = user.InvestorName;
            userResponse.Avatar = user.PhotoPath;
            userResponse.AccountName =user.InvestorName;
             userResponse.Area = "Investor";
        ...
        private async Task<OperationResult> GenerateOperationResultForUserForGotPasswordAsync(Investor user, string email)
        {
        ...
        {
            var employee = await _repo.FindAll(x => x.Status == 1).AsNoTracking().FirstOrDefaultAsync(x => x.InvestorEmail == email);
