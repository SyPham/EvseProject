...
{
    public interface IAuthLandLordService : IScopeService
    {
        Task<LandLord> Login(string username, string password);
        Task LogOut();
    ...
    public class AuthLandLordService : IAuthLandLordService
    {
...
,
        ...
        public async Task<LandLord> Login(string username, string password)
        {
        ...
        {
            string token = _httpContextAccessor.HttpContext.Request.Headers["AuthLandLordorization"];
     ...
     public async  Task<OperationResult> ChangePassword(ChangePasswordDto reset) {
        ...
        private async Task<OperationResult> GenerateOperationResultForUserAsync(LandLord user, string password)
        {
            var claims = new[]
            {
        ...
        private async Task<OperationResult> GenerateOperationResultForUserForGotPasswordAsync(LandLord user, string email)
        {
        ...
        {
            var employee = await _repo.FindAll(x => x.Status == 1).AsNoTracking().FirstOrDefaultAsync(x => x.LandLordEmail == email);
