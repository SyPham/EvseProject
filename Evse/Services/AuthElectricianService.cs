...
{
    public interface IAuthElectricianService : IScopeService
    {
        Task<Electrician> Login(string username, string password);
        Task LogOut();
    ...
    public class AuthElectricianService : IAuthElectricianService
    {
...
,
        ...
        public async Task<Electrician> Login(string username, string password)
        {
        ...
        {
            string token = _httpContextAccessor.HttpContext.Request.Headers["AuthElectricianorization"];
     ...
     public async  Task<OperationResult> ChangePassword(ChangePasswordDto reset) {
        ...
        private async Task<OperationResult> GenerateOperationResultForUserAsync(Electrician user, string password)
        {
            var claims = new[]
            {
            ...
            await _unitOfWork.SaveChangeAsync();
            var userResponse = _mapper.Map<UserElectricianForDetailDto>(user);
             userResponse.Mobile = user.ElectricianMobile;
            userResponse.Email = user.ElectricianEmail;
            userResponse.FullName = user.ElectricianName;
            userResponse.NickName = user.ElectricianName;
            userResponse.Avatar = user.PhotoPath;
            userResponse.AccountName =user.ElectricianName;
             userResponse.Area = "Electrician";
        ...
        private async Task<OperationResult> GenerateOperationResultForUserForGotPasswordAsync(Electrician user, string email)
        {
        ...
        {
            var employee = await _repo.FindAll(x => x.Status == 1).AsNoTracking().FirstOrDefaultAsync(x => x.ElectricianEmail == email);
